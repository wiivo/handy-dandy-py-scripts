...
parser.add_argument(
    "-d", "--dry", action="store_true", default=False, help="do a dry run"
)
parser.add_argument(
    "-a",
    "--all",
...
for file in dir_list:
    ...
    if os.path.isfile(file_path):
        if (
            re.search(r"\.sfk\d?$", file) and (file[:-5] not in dir_list or args.all)
        ) or (
            re.search(r"\.veg.bak$", file) and (file[:-4] not in dir_list or args.all)
        ):
            if not args.dry:
                os.remove(file_path)
            print(file + " deleted")
